...
func _ready(): # Funcion inicial para detectar colisiones
	add_to_group(game.GROUP_BIRDS)
	connect("body_entered", self, "_on_body_entered")
	pass
func _physics_process(delta):	# Funcion de control de logicas de comportamiento del pajaro
	state.update(delta)
func _input(event): # Deteccion de entradas de juego y transferencia al estado
	state.input(event)
	pass
func _on_body_entered(other_body): # Funcion para detectar y manejar las colisiones
	if state.has_method("on_body_entered"):
		state.on_body_entered(other_body)
	pass
func set_state(new_state): # Funcion de cambio de estado del pajaro
	...
	pass
func get_state(): # Funcion de estado actual del pajaro basado en la clase del objeto state
...
# Clases de administracion de estados de juego:

class FlyingState: # Clase de estado de vuelo del pajaro
	var bird
...
class FlappingState: # Clase de estado de aleteo o vuelo del pajaro
	var bird
	func _init(bird):
		self.bird = bird
		bird.set_linear_velocity(Vector2(bird.speed, bird.get_linear_velocity().y))
...
class HitState: # Clase de estado de colision
	var bird
	func _init(bird):
		self.bird = bird
...
class GroundedState: # Clase de manejo de colision con suelo del pajaro
	var bird
	func _init(bird):
		self.bird = bird
		bird.set_linear_velocity(Vector2(0,0))
